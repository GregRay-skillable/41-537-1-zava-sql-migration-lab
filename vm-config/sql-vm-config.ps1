        ...
        Script RestoreToyStore {
            ...
            GetScript = {
                $dbExists = Invoke-Sqlcmd -ServerInstance Localhost -Database "master" -TrustServerCertificate -Query "
                    SELECT name FROM sys.databases WHERE name = 'ZavaStore'"
                @{ Result = $dbExists }
            }
            TestScript = {
                $dbExists = Invoke-Sqlcmd -ServerInstance Localhost -Database "master" -TrustServerCertificate -Query "
            ...
            SetScript = {
                $backupFileName = Split-Path $using:DbBackupFileUrl -Leaf
                $dbDestination = "C:\$backupFileName"
                Write-Verbose "Restoring ZavaStore database..."
                ...
                foreach ($file in $files) {
                        ...
                        $relocateFiles += New-Object Microsoft.SqlServer.Management.Smo.RelocateFile(
                            $file.LogicalName, "C:\Database\Data\ZavaStore.mdf")
                    }
                    elseif ($file.Type -eq 'L') {
                        $relocateFiles += New-Object Microsoft.SqlServer.Management.Smo.RelocateFile(
                            $file.LogicalName, "C:\Database\Logs\ZavaStore.ldf")
                    }
                }
                ...
                Write-Verbose "ZavaStore database restored."
            }
        }
        ...
        Script SetToyStoreRecoveryMode {
            ...
            GetScript = {
                $model = Invoke-Sqlcmd -ServerInstance Localhost -Database "master" -TrustServerCertificate -Query "
                    SELECT recovery_model_desc FROM sys.databases WHERE name = 'ZavaStore'"
                @{ Result = $model }
            }
            TestScript = {
                $model = Invoke-Sqlcmd -ServerInstance Localhost -Database "master" -TrustServerCertificate -Query "
            ...
            SetScript = {
                Write-Verbose "Setting ZavaStore recovery model to FULL and running backup..."
                Invoke-Sqlcmd -ServerInstance Localhost -Database "master" -TrustServerCertificate -Query "
                    ALTER DATABASE ZavaStore SET RECOVERY FULL"
                Backup-SqlDatabase -ServerInstance Localhost -Database ZavaStore
                Write-Verbose "ZavaStore recovery model set to FULL."
            }
        }


        # Create full backup of ZavaStore database
        Script BackupToyStoreDatabase {
            DependsOn = '[Script]RestoreToyStore'
            GetScript = {
                $backupPath = "C:\Database\Backup\ZavaStore_full.bak"
            ...
            SetScript = {
                Write-Verbose "Performing full backup of ZavaStore database..."
                $backupDir = "C:\Database\Backup"
                $backupPath = "$backupDir\ZavaStore_full.bak"

                if (-not (Test-Path $backupDir)) {
                    New-Item -ItemType Directory -Path $backupDir -Force | Out-Null
                }

                $query = "BACKUP DATABASE [ZavaStore] TO DISK = N'$backupPath' WITH INIT, FORMAT, NAME = 'ZavaStore-FullBackup';"
